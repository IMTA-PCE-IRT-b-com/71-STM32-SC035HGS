...
error_bno bno055_read_regs_IT(bno055_t imu, u8 reg, uint8_t* buf, uint32_t size)
{
    HAL_StatusTypeDef ret;

    // Lancer lՎcriture non bloquante (adresse du registre)
    ret = HAL_I2C_Master_Transmit_IT(imu.i2c, imu.addr, &reg, 1);
    if(ret != HAL_OK) return BNO_ERR_I2C;
