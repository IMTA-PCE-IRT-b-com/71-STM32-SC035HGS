...
SENSOR_DRV_ENTRY_IMPL_BEGIN_EX(sc035hgs);
...
#define SENSOR_DATAPREC     CUS_DATAPRECISION_8    //CUS_DATAPRECISION_8, CUS_DATAPRECISION_10
#define SENSOR_DATAMODE     CUS_SEN_8TO12_7000
#define SENSOR_BAYERID     CUS_BAYER_BG // (CUS_BAYER_BG + 1)            //CUS_BAYER_GB, CUS_BAYER_GR, CUS_BAYER_BG, CUS_BAYER_RG
...
static const I2C_ARRAY Sensor_id_table[] =
{
    {0x3107, 0xcc},
    {0x3108, 0x1a},
};

static const I2C_ARRAY Sensor_init_table[] =
{
{0x0103,0x01},
{0x0100,0x00},
{0x36e9,0x80},
...
/////////// function definition ///////////////////
#if SENSOR_DBG == 1
//#define printk(args...) LOGD(args)
//#define printk(args...) LOGE(args)
...
static int pCus_poweroff(ms_cus_sensor *handle, u32 idx)
{
    // power/reset low
    ISensorIfAPI *sensor_if = handle->sensor_if_api;
    sc035hgs_params *params = (sc035hgs_params *)handle->private_data;
    printk("[%s] power low\n", __FUNCTION__);
    sensor_if->PowerOff(idx, handle->pwdn_POLARITY);
    sensor_if->Reset(idx, handle->reset_POLARITY);
...
static int sc035hgs_SetPatternMode(ms_cus_sensor *handle,u32 mode)
{
  int i;
  printk("\n\n[%s], mode=%d \n", __FUNCTION__, mode);
...
static int pCus_init_linear_30W30fps(ms_cus_sensor *handle)
{
    ...
    for(i=0;i< ARRAY_SIZE(Sensor_init_table);i++)
    {
		...
		if(Sensor_init_table[i].reg==0x4418 || Sensor_init_table[i].reg==0x4419)	SENSOR_MSLEEP(10);
        ...
        else
        {
            ...
            {
                cnt++;
                printk("Sensor_init_table -> Retry %d...\n",cnt);
                if(cnt>=10)
                {
                    printk("[%s:%d]Sensor init fail!!\n", __FUNCTION__, __LINE__);
                    return FAIL;
                }
                SENSOR_MSLEEP(10);
            }
   ...
   printk("cur_orien %s pCus_SetOrien %x\n",__FUNCTION__, params->cur_orien);
...
static int pCus_GetAEUSecs(ms_cus_sensor *handle, u32 *us) {
  ...
  printk("[%s] sensor expo lines/us %d, %dus\n", __FUNCTION__, lines, *us);
  return rc;
}
...
static int pCus_SetAEGain(ms_cus_sensor *handle, u32 gain) {
    ...
    u16 gain_reg_0x3314, gain_reg_0x3317, gain_reg_0x3631, gain_reg_0x3630;
    int i;
	#if ENABLE_NR
    ...
    if (gain < 2048) {
        // start again  2 * 1024
        gain_reg_0x3314 = 0x41;
        gain_reg_0x3317 = 0x08;
        gain_reg_0x3631 = 0x58;
        gain_reg_0x3630 = 0x4a;
    } else if (gain < 4096) {
        // 4 * 1024
        gain_reg_0x3314 = 0x6f;
        gain_reg_0x3317 = 0x10;
        gain_reg_0x3631 = 0x48;
        gain_reg_0x3630 = 0x4c;
    } else {
        // 8 * 1024
        gain_reg_0x3314 = 0x76;
        gain_reg_0x3317 = 0x14;
        gain_reg_0x3631 = 0x48;
        gain_reg_0x3630 = 0x4c;
    }

    params->tGain_reg[3].data = gain_reg_0x3630;    // 0x3630
    params->tGain_reg[2].data = gain_reg_0x3631;    // 0x3631
    params->tGain_reg[1].data = gain_reg_0x3317;    // 0x3317
    params->tGain_reg[0].data = gain_reg_0x3314;    // 0x3314
...
static int sc035hgs_GetShutterInfo(struct __ms_cus_sensor* handle,CUS_SHUTTER_INFO *info)
{
...
static int cus_camsensor_init_handle(ms_cus_sensor* drv_handle) {
   ms_cus_sensor *handle = drv_handle;
    ...
    params = (sc035hgs_params *)handle->private_data;
    ...
    //    sensor model ID                           //
    ////////////////////////////////////
    strcpy(handle->model_id,"sc035hgs_PARL");

    ////////////////////////////////////
    //    sensor interface info       //
    ////////////////////////////////////
    //printk("[%s] entering function with id %d\n", __FUNCTION__, id);
    handle->isp_type    = SENSOR_ISP_TYPE;  //ISP_SOC;
    //handle->data_fmt    = SENSOR_DATAFMT;   //CUS_DATAFMT_YUV;
    handle->sif_bus     = CUS_SENIF_BUS_PARL;//SENSOR_IFBUS_TYPE;//CUS_SENIF_BUS_PARL;
   ...
   handle->interface_attr.attr_parallel.parallel_data_format = CUS_SEN_INPUT_FORMAT_RGB;
   handle->interface_attr.attr_parallel.parallel_yuv_order = 0;
   handle->interface_attr.attr_parallel.parallel_vif_prec = CUS_DATAPRECISION_12;
   handle->interface_attr.attr_parallel.u8RgbOrYuv = CUS_SEN_INPUT_FORMAT_RGB;
   ...
   // handle->interface_attr.attr_mipi.mipi_hdr_virtual_channel_num = 0; //Short frame
    ...
    sprintf(handle->video_res_supported.res[0].strResDesc, "640*480@30fps");
    ...
    handle->pCus_sensor_SetPatternMode = sc035hgs_SetPatternMode;
    ///////////////////////////////////////////////////////
    // AE
    ///////////////////////////////////////////////////////
    // unit: micro seconds
    ...
    handle->pCus_sensor_GetShutterInfo = sc035hgs_GetShutterInfo;
    params->expo.vts=vts_30fps;
    params->expo.fps = 30;
    params->expo.line= 15783;
    params->reg_dirty = false;
    params->nr_dirty = false;
...
SENSOR_DRV_ENTRY_IMPL_END_EX( sc035hgs,
